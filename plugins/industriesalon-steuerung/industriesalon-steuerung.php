 ...
 * Description: Zentrale Seiteneinstellungen für Industriesalon: Öffnungszeiten, Bürozeiten, Kontakt, Adresse, Mission Statement, Barrierefreiheit und Preise mit wiederverwendbarer Ausgabe per PHP und Gutenberg-Blöcke.
 * Version: 0.3.0
 * Author: OpenAI
...
final class Industriesalon_Steuerung {
    ...
    /**
     * Keep structured editorial post types on the classic editor while
     * preserving REST exposure for frontend queries and projection layers.
     */
    public function structured_editor_post_types(): array {
        return [
            'veranstaltung',
            'ausstellung',
            'projekt',
    ...
    public function is_structured_editor_post_type(string $post_type): bool {
        return in_array(sanitize_key($post_type), $this->structured_editor_post_types(), true);
    }

    public function apply_structured_content_editor_policy(bool $use_block_editor, string $post_type): bool {
        if ($this->is_structured_editor_post_type($post_type)) {
            return false;
        }
    ...
    public function enqueue_admin_assets(string $hook): void {
        ...
        $screen = get_current_screen();
        if (!($screen instanceof WP_Screen) || !$this->is_structured_editor_post_type((string) $screen->post_type)) {
            return;
        }

        wp_enqueue_style(
            'iss-structured-editor-policy',
            plugins_url('assets/editor-policy.css', __FILE__),
            [],
            self::VERSION
        );
    }
    ...
    private function render_admin_notice(): void {
        // Read-only admin notice state from a redirect query arg.
    ...
    public function register_blocks(): void {
        ...
        $blocks_js_path = plugin_dir_path(__FILE__) . 'assets/blocks.js';
        $blocks_js_version = file_exists($blocks_js_path) ? (string) filemtime($blocks_js_path) : self::VERSION;

        // One shared editor script keeps the remaining block controls
        // consistent across field, hours, contact, and visit-info outputs.
        wp_register_script(
            'iss-control-blocks',
        ...
        register_block_type('industriesalon/visit-info', [
            ...
            'attributes'      => [
                ...
                'show_contact_phone' => ['type' => 'boolean', 'default' => true],
                'show_contact_email' => ['type' => 'boolean', 'default' => true],
                'show_contact_booking_email' => ['type' => 'boolean', 'default' => false],
                'show_contact_person' => ['type' => 'boolean', 'default' => false],
                'show_contact_website' => ['type' => 'boolean', 'default' => true],
            ],
        ]);
    ...
    public function render_visit_info(array $attributes = []): string {
        ...
        $panel_skin_class = in_array($skin, ['compact', 'menu'], true)
            ? ' iss-info-panel--skin-' . $skin
            : '';
        ...
        $show_contact_phone = array_key_exists('show_contact_phone', $attributes) ? (bool) $attributes['show_contact_phone'] : true;
        $show_contact_email = array_key_exists('show_contact_email', $attributes) ? (bool) $attributes['show_contact_email'] : true;
        $show_contact_booking_email = array_key_exists('show_contact_booking_email', $attributes) ? (bool) $attributes['show_contact_booking_email'] : false;
        $show_contact_person = array_key_exists('show_contact_person', $attributes) ? (bool) $attributes['show_contact_person'] : false;
        $show_contact_website = array_key_exists('show_contact_website', $attributes) ? (bool) $attributes['show_contact_website'] : true;
        ...
        $contact_fields = [];
        if ($show_contact_phone) {
            $contact_fields[] = 'phone';
        }
        if ($show_contact_email) {
            $contact_fields[] = 'email';
        }
        if ($show_contact_booking_email) {
            $contact_fields[] = 'booking_email';
        }
        if ($show_contact_person) {
            $contact_fields[] = 'contact_person';
        }
        if ($show_contact_website) {
            $contact_fields[] = 'website';
        }

        if ($show_contact) {
            $rows[] = $this->build_visit_info_contact_row($contact_fields);
        }
        ...
        <?php endif; ?>
            <section class="iss-info-panel iss-info-panel--<?php echo esc_attr($accent); ?> iss-info-panel--surface-<?php echo esc_attr($surface); ?><?php echo esc_attr($panel_skin_class); ?> iss-control-info-panel" data-skin="<?php echo esc_attr($skin); ?>">
                <div class="wp-block-columns iss-info-panel__grid">
                    <div class="wp-block-column iss-info-panel__title-col">
                        <div class="iss-heading iss-heading--uncaged iss-info-panel__heading">
                            <?php if ($kicker !== '') : ?>
                                <p class="iss-kicker iss-control-info-panel__kicker"><?php echo esc_html($kicker); ?></p>
                            <?php endif; ?>
                            <?php if ($title !== '') : ?>
                                <h2 class="iss-heading__title iss-info-panel__title"><?php echo esc_html($title); ?></h2>
                            <?php endif; ?>
                            <?php if ($intro !== '') : ?>
                                <p class="iss-heading__text iss-info-panel__intro"><?php echo nl2br(esc_html($intro)); ?></p>
                            <?php endif; ?>
                        </div>
                    </div>
                    <div class="wp-block-column iss-info-panel__rows-col">
    ...
    public function render_contact(string $title = '', array $options = []): string {
        ...
        if ($variant === 'compact-row') {
            $fields = $this->normalize_compact_contact_fields($options['fields'] ?? []);
            $parts = [];
            foreach ($fields as $field) {
                $item = $this->render_compact_contact_item($field, $contact);
                if ($item !== '') {
                    $parts[] = $item;
                }
            }
    ...
    private function normalize_compact_contact_fields($fields): array {
        if (! is_array($fields)) {
            $fields = ['phone', 'email', 'website'];
        }

        $allowed = ['phone', 'email', 'booking_email', 'contact_person', 'website'];
        $normalized = [];
        foreach ($fields as $field) {
    ...
    private function build_visit_info_contact_row(array $fields): string {
        $contact = get_option(self::OPTION_CONTACT, $this->default_contact());
        $fields = $this->normalize_compact_contact_fields($fields);
        $parts = [];

        foreach ($fields as $field) {
            $item = $this->render_visit_info_contact_field($field, $contact);
            if ($item !== '') {
                $parts[] = $item;
            }
        }
        ...
        return $this->render_visit_info_row_html(
            'contact',
            '<strong>' . esc_html__('Kontakt:', 'industriesalon-steuerung') . '</strong>' . implode('', $parts)
        );
    }

    private function render_visit_info_contact_field(string $field, array $contact): string {
        switch ($field) {
            case 'phone':
                ...
                return '<span class="iss-control-info-panel__subline iss-control-info-panel__subline--contact"><span class="iss-control-info-panel__subline-label">' . esc_html__('Tel.', 'industriesalon-steuerung') . '</span> <a href="' . esc_url('tel:' . $this->normalize_phone_href((string) $contact['phone'])) . '">' . esc_html((string) $contact['phone']) . '</a></span>';
            ...
            case 'email':
                ...
                return '<span class="iss-control-info-panel__subline iss-control-info-panel__subline--contact"><span class="iss-control-info-panel__subline-label">' . esc_html__('E-Mail', 'industriesalon-steuerung') . '</span> <a href="' . esc_url('mailto:' . antispambot((string) $contact['email'])) . '">' . esc_html((string) $contact['email']) . '</a></span>';
            ...
            case 'booking_email':
                ...
                return '<span class="iss-control-info-panel__subline iss-control-info-panel__subline--contact"><span class="iss-control-info-panel__subline-label">' . esc_html__('Buchung', 'industriesalon-steuerung') . '</span> <a href="' . esc_url('mailto:' . antispambot((string) $contact['booking_email'])) . '">' . esc_html((string) $contact['booking_email']) . '</a></span>';
            ...
            case 'contact_person':
                ...
                return '<span class="iss-control-info-panel__subline iss-control-info-panel__subline--contact"><span class="iss-control-info-panel__subline-label">' . esc_html__('Ansprechperson', 'industriesalon-steuerung') . '</span> ' . esc_html((string) $contact['contact_person']) . '</span>';
            ...
            case 'website':
                ...
                return '<span class="iss-control-info-panel__subline iss-control-info-panel__subline--contact"><span class="iss-control-info-panel__subline-label">' . esc_html__('Website', 'industriesalon-steuerung') . '</span> <a href="' . esc_url($this->normalize_public_url((string) $contact['website'])) . '" target="_blank" rel="noopener noreferrer">' . esc_html((string) $contact['website']) . '</a></span>';
        }

        return '';
    }

    private function render_compact_contact_item(string $field, array $contact): string {
        switch ($field) {
            case 'phone':
                if (empty($contact['phone'])) {
                    return '';
                ...
                return '<span class="iss-contact-card__item iss-contact-card__item--phone"><span class="iss-contact-card__item-label">' . esc_html__('Tel.', 'industriesalon-steuerung') . '</span> <a href="' . esc_url('tel:' . $this->normalize_phone_href((string) $contact['phone'])) . '">' . esc_html((string) $contact['phone']) . '</a></span>';
            ...
            case 'email':
                ...
                return '<span class="iss-contact-card__item iss-contact-card__item--email"><span class="iss-contact-card__item-label">' . esc_html__('E-Mail', 'industriesalon-steuerung') . '</span> <a href="' . esc_url('mailto:' . antispambot((string) $contact['email'])) . '">' . esc_html((string) $contact['email']) . '</a></span>';
            ...
            case 'booking_email':
                ...
                return '<span class="iss-contact-card__item iss-contact-card__item--booking-email"><span class="iss-contact-card__item-label">' . esc_html__('Buchung', 'industriesalon-steuerung') . '</span> <a href="' . esc_url('mailto:' . antispambot((string) $contact['booking_email'])) . '">' . esc_html((string) $contact['booking_email']) . '</a></span>';
            ...
            case 'contact_person':
                ...
                return '<span class="iss-contact-card__item iss-contact-card__item--contact-person"><span class="iss-contact-card__item-label">' . esc_html__('Ansprechperson', 'industriesalon-steuerung') . '</span> ' . esc_html((string) $contact['contact_person']) . '</span>';
            ...
            case 'website':
                ...
                return '<span class="iss-contact-card__item iss-contact-card__item--website"><span class="iss-contact-card__item-label">' . esc_html__('Website', 'industriesalon-steuerung') . '</span> <a href="' . esc_url($this->normalize_public_url((string) $contact['website'])) . '" target="_blank" rel="noopener noreferrer">' . esc_html((string) $contact['website']) . '</a></span>';
        }
    ...
    private function normalize_output_skin(string $skin): string {
        ...
        $allowed = ['default', 'light', 'dark', 'muted', 'front', 'accent-red', 'menu', 'compact'];
