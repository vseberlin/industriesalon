...
function iss_fuehrung_get_offer_catalog_group_keys($post_id) {
    ...
    $haystack = strtolower(remove_accents(implode(' ', array_filter([
        ...
        $target_group,
        trim((string) get_the_excerpt($post_id)),
...
function iss_fuehrung_render_offer_catalog_item($post_id) {
    ...
    $card_classes = ['iss-tour-offer-catalog__card'];
    $item_classes = ['iss-quiet-card', 'iss-tour-offer-catalog__quiet-card'];

    if ($has_thumbnail) {
        $card_classes[] = 'iss-tour-offer-catalog__card--media';
    } else {
        $card_classes[] = 'iss-tour-offer-catalog__card--text';
    }

    if ($kicker === '') {
        if ($state['mode'] === 'calendar') {
            $kicker = __('Öffentlicher Rundgang', 'iss-fuehrungen');
        } elseif ($state['mode'] === 'hybrid') {
            $kicker = __('Öffentlich + Anfrage', 'iss-fuehrungen');
        } elseif ($state['mode'] === 'on_demand') {
            $kicker = __('Individuell buchbar', 'iss-fuehrungen');
        } else {
            $kicker = __('Führung', 'iss-fuehrungen');
        }
    }
    ...
    if (!empty($meta)) {
        $summary_parts[] = implode(' · ', array_slice($meta, 0, 2));
    }

    if (!empty($state['value'])) {
        $summary_parts[] = sprintf('%s: %s', (string) $state['label'], (string) $state['value']);
    }
    ...
    if ($has_thumbnail) {
        echo '<figure class="iss-quiet-card__media"><a href="' . esc_url(get_permalink($post_id)) . '">';
        echo get_the_post_thumbnail($post_id, 'large');
        echo '</a></figure>';
    }

    echo '<div class="iss-quiet-card__body">';
    echo '<p class="iss-quiet-card__kicker">' . esc_html($kicker) . '</p>';
    echo '<h4 class="iss-quiet-card__title"><a href="' . esc_url(get_permalink($post_id)) . '">' . esc_html(get_the_title($post_id)) . '</a></h4>';

    if ($excerpt !== '') {
        echo '<p class="iss-quiet-card__text">' . esc_html($excerpt) . '</p>';
    }

    if ($summary_parts) {
        echo '<p class="iss-quiet-card__meta">' . esc_html(implode(' · ', $summary_parts)) . '</p>';
    }

    if (!empty($action['url']) && !empty($action['label'])) {
        echo '<p class="iss-quiet-card__action"><a href="' . esc_url((string) $action['url']) . '">' . esc_html((string) $action['label']) . '</a></p>';
    }
...
function iss_fuehrung_render_offer_catalog_block($attributes = [], $content = '') {
    ...
    $available_groups = array_filter(array_keys($groups), static function ($group_key) use ($grouped_posts) {
    ...
    $script_path = ISS_FUEHRUNGEN_PATH . 'assets/tour-offer-catalog.js';
    if (file_exists($script_path)) {
        wp_enqueue_script(
            'iss-fuehrungen-offer-catalog',
            ISS_FUEHRUNGEN_URL . 'assets/tour-offer-catalog.js',
            [],
    ...
    $default_group = in_array('oeffentlich', $available_groups, true)
        ? 'oeffentlich'
    ...
    foreach ($available_groups as $group_key) {
        $group = $groups[$group_key];
        $button_id = 'iss-tour-offer-catalog-tab-' . sanitize_html_class($group_key);
        echo '<button type="button" class="iss-tour-offer-catalog__filter' . ($group_key === $default_group ? ' is-active' : '') . '" id="' . esc_attr($button_id) . '" data-offer-target="' . esc_attr($group_key) . '" aria-selected="' . ($group_key === $default_group ? 'true' : 'false') . '">';
        echo esc_html((string) $group['label']);
        echo '</button>';
    }

    echo '</div>';
    echo '<div class="iss-tour-offer-catalog__panels">';
    ...
    foreach ($available_groups as $group_key) {
        ...
        $group = $groups[$group_key];
        echo '<section class="iss-tour-offer-catalog__panel iss-tour-offer-catalog__panel--' . esc_attr(sanitize_html_class($group_key)) . '" data-offer-panel="' . esc_attr($group_key) . '">';
        echo '<div class="iss-tour-offer-catalog__panel-head">';
