...
function iss_register_upsert_place(array $item, bool $dry_run, array &$stats, array &$messages): void
{
    ...
    foreach ($item['meta'] as $key => $value) {
        if (
            $existing_post_id > 0
            && in_array($key, iss_register_coordinate_meta_keys(), true)
            && !is_array($value)
            && $value === ''
        ) {
            continue;
...
function iss_register_render_import_page(): void
{
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['iss_register_import_action'])) {
        $action = sanitize_key((string) wp_unslash($_POST['iss_register_import_action']));
    ...
    echo '<p>Geocoding läuft über Nominatim und schreibt die gefundenen Werte in <code>lat</code>, <code>lng</code> und <code>coordinates_accuracy</code>. Leere Koordinaten aus dem JSON-Import überschreiben bestehende geocodierte Werte nicht mehr.</p>';
    echo '<form method="post">';
    wp_nonce_field('iss_register_run_geocode', 'iss_register_geocode_nonce');
    echo '<input type="hidden" name="iss_register_import_action" value="geocode">';
    echo '<table class="form-table" role="presentation"><tbody>';
