...
function iss_register_get_related_objects_for_place(int $place_id, int $limit = 6): array
{
    ...
    if (function_exists('iss_graph_get_archive_objects_for_place')) {
        $items = iss_graph_get_archive_objects_for_place($place_id, $limit);
        if ($items) {
            return $items;
        }
    }

    if (!function_exists('iss_relations_get_place_term_ids')) {
        return [];
    }
