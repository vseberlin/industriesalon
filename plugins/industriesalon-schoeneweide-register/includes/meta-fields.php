...
function iss_register_get_meta_schema(): array
{
    return [
        ...
        'potential_note' => ['type' => 'string', 'label' => 'Potenzial', 'input' => 'textarea', 'show_in_rest' => true, 'admin' => true],
...
function iss_register_string_array_rest_schema(): array
{
    return [
        'type' => 'array',
        'items' => [
...
function iss_register_register_meta_fields(): void
{
    foreach (iss_register_get_meta_schema() as $key => $field) {
        ...
        } elseif ($show_in_rest && ($field['type'] ?? '') === 'array') {
            $show_in_rest = [
                'schema' => iss_register_string_array_rest_schema(),
            ];
        }
...
function iss_register_get_meta_box_groups(): array
{
    return [
        ...
        'atlas_public' => [
            ...
            'fields' => ['history_short', 'current_use', 'potential_note', 'risk_note', 'industry', 'source_summary', 'tags'],
        ],
        'atlas_media' => [
