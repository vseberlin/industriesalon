(function () {
  var recent = [];
  var markPattern = /^iss-(ink|mark)-([0-9a-f]{6}|preset-[a-z0-9-]+)$/;
  function colourValue(key, palette) {
    if (/^[0-9a-f]{6}$/.test(key)) { return '#' + key; }
    var choice = (palette || (window.issEditorialAdmin || {}).textPalette || []).find(function (item) { return 'preset-' + item.slug === key; });
    return choice ? choice.color : '';
  }
  function permittedMark(name) {
    var match = name.match(markPattern);
    return !!(match && colourValue(match[2]));
  }
  ...
  function sanitize(value, profile) {
    ...
    function append(node, target) {
      ...
      if (keep && tag === 'span') {
        var marks = Array.from(node.classList).filter(function (name) { return permittedMark(name); });
  ...
  function hasUnsupportedMarkup(value, profile) {
    ...
    return Array.from(template.content.querySelectorAll('*')).some(function (node) {
      ...
      return Array.from(node.attributes).some(function (attr) {
        if (tag === 'span' && attr.name === 'class') { return Array.from(node.classList).some(function (name) { return !permittedMark(name); }); }
    ...
    return {
      ...
      setup: function (editor) {
        ...
        function register(kind, key) {
          var color = colourValue(key, palette);
          var name = 'iss-' + kind + '-' + key;
          if (!registered[name]) {
            editor.formatter.register(name, { inline: 'span', classes: name, exact: true });
            registered[name] = true;
            editor.dom.addStyle('.' + name + '{' + (kind === 'ink' ? 'color' : 'background-color') + ':' + color + '}');
          }
          return name;
        }
        function discover() {
          Array.from(editor.getBody().querySelectorAll('span[class]')).forEach(function (span) {
            Array.from(span.classList).forEach(function (name) { var match = name.match(markPattern); if (match && colourValue(match[2], palette)) { register(match[1], match[2]); } });
          });
        }
        function colourPanel(kind) {
          ...
          function apply(key) {
            ...
            editor.undoManager.transact(function () {
              ...
              if (key) { editor.formatter.apply(register(kind, key)); }
            });
            panel.hidden = true;
            editor.nodeChanged(); editor.fire('change');
            if (/^[0-9a-f]{6}$/.test(key)) { recent = [key].concat(recent.filter(function (item) { return item !== key; })).slice(0, 5); }
          }
          ...
          function swatch(name, key) {
            var color = colourValue(key, palette);
            if (!/^#[0-9a-f]{6}$/.test(color)) { return; }
            var hex = color.slice(1);
            var choice = button(name, function () { apply(key); });
          ...
          palette.forEach(function (color) { swatch(color.name, 'preset-' + color.slug); });
          recent.forEach(function (hex) { swatch('Zuletzt: #' + hex, hex); });
          panel.appendChild(swatchStyle);
          panel.appendChild(choices);
          var guidance = document.createElement('p'); guidance.className = 'description';
          guidance.textContent = 'Website-Farben folgen der Farbpalette. Eigene Farben bleiben fest. Hellen Text auf dunklem Hintergrund verwenden; bei Bildern den Kontrast in der Live-Vorschau prüfen.';
          panel.appendChild(guidance);
          var customGroup = document.createElement('details');
          var summary = document.createElement('summary'); summary.textContent = 'Eigene Farbe'; customGroup.appendChild(summary); panel.appendChild(customGroup);
          var custom = document.createElement('label'); custom.textContent = 'Eigene Farbe ';
          var picker = document.createElement('input'); picker.type = 'color'; picker.value = palette.length ? palette[0].color : '#000000'; custom.appendChild(picker); customGroup.appendChild(custom);
          var hexLabel = document.createElement('label'); hexLabel.textContent = 'Hex-Farbwert ';
          var hexInput = document.createElement('input'); hexInput.type = 'text'; hexInput.value = picker.value; hexInput.placeholder = '#123abc'; hexInput.maxLength = 7; hexLabel.appendChild(hexInput); customGroup.appendChild(hexLabel);
          ...
          customGroup.appendChild(button('Farbe anwenden', function () {
