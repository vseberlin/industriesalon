...
class ISS_WF_Import_Archive_Browser_Service
{
    ...
    private array $search_post_ids_cache = [];
    ...
    public function get_term_options(string $taxonomy, int $limit = 0): array
    {
        ...
        // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Archive browser facets read the archive projection table.
    ...
    protected function get_grouped_source_counts(array $state): array
    {
        ...
        // phpcs:disable WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Archive source counts are projection-table aggregates.
    ...
    protected function count_objects(array $state, array $config): int
    {
        ...
        // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Archive object counts are projection-table aggregates.
    ...
    protected function build_where_sql(array $state, array $config): array
    {
        ...
        $taxonomy_filters = [
            ...
            ISS_WF_IMPORT_DECADE_TAXONOMY => ['enabled' => !empty($config['include_decade']), 'slug' => (string) $state['decade']],
        ];
        ...
        if (!empty($config['include_search']) && $state['search'] !== '') {
            $post_ids = $this->get_search_post_ids($state['search']);
            if (!$post_ids) {
                $where[] = '1 = 0';
            } else {
                $where[] = 'p.ID IN (' . implode(', ', array_fill(0, count($post_ids), '%d')) . ')';
    ...
    public function build_query_args(array $state, int $per_page): array
    {
        ...
        if ($state['decade'] !== '') {
            $tax_query[] = [
                'taxonomy' => ISS_WF_IMPORT_DECADE_TAXONOMY,
                'field' => 'slug',
                'terms' => [$state['decade']],
            ];
        }
        ...
        if ($tax_query) {
            ...
            // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_tax_query -- Archive taxonomy facets are the browser's domain filters.
    ...
    public function normalize_state(array $state): array
    {
        return [
            ...
            'decade' => sanitize_title((string) ($state['decade'] ?? '')),
            'search' => sanitize_text_field((string) ($state['search'] ?? '')),
            'page' => max(1, (int) ($state['page'] ?? 1)),
        ];
    }

    protected function get_search_post_ids(string $search): array
    {
        ...
        if (isset($this->search_post_ids_cache[$search])) {
            return $this->search_post_ids_cache[$search];
        }

        $post_ids = function_exists('iss_search_public_post_ids')
            ? iss_search_public_post_ids($search, [
                'types' => ['archive_object'],
                'limit' => 1000,
            ])
            : [];

        $post_ids = array_values(array_unique(array_filter(array_map('absint', $post_ids))));
        $this->search_post_ids_cache[$search] = $post_ids;
...
if (defined('WP_CLI') && WP_CLI) {
    class ISS_WF_Import_Archive_Browser_CLI_Command
    {
        public function verify(): void
        {
            ...
            if (is_array($decade_terms) && !empty($decade_terms[0]) && $decade_terms[0] instanceof WP_Term) {
                $states['decade'] = ['decade' => $decade_terms[0]->slug, 'page' => 1];
            }
            ...
            foreach ([
                ...
                [ISS_WF_IMPORT_DECADE_TAXONOMY, 0],
            ] as [$taxonomy, $limit]) {
