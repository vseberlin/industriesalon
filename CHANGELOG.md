...
- Added the first read-only greenfield facade under `/wp-json/iss/v1`:
  - exposed contract, entity list/detail, occurrence list, timeline view, tour-slot read view, and search endpoints without removing the existing `iss-search/v1`, `is-tours/v1`, `iss-programm/v1`, `iss-graph/v1`, or plugin-owned query APIs
  - kept the facade as a formatter/delegator over the existing graph, search, occurrence, programme timeline, and tour-slot services so it introduces no new storage owner
  ...
  - added `wp iss-graph facade-tour-slots-compare` to compare legacy `/is-tours/v1/slots` output with the `/iss/v1/tour-slots` read view before tour-slot consumers switch routes
  ...
  - switched the public tour calendar slot reader from legacy `/is-tours/v1/slots` to `/iss/v1/tour-slots`; booking submissions remain on `/is-tours/v1/book`
- Started the greenfield refactor path as a local checkpoint:
  ...
  - cleaned the local Ausstellung availability data after backup `ops/content-backups/2026-06-11-before-ausstellung-availability-cleanup.sql`, with replayable SQL in `ops/sql/2026-06-11-ausstellung-availability-cleanup.sql`: `Kinder im Werk` and `Frauen im Werk für Fernmeldewesen` are Digital exhibitions through 2026-12-31, while `Ostberliner Zeitreisen` and `Die laufende Produktion` are drafts pending later review
  ...
  - exposed the hidden `ausstellung_typ` taxonomy to the editor REST API and added explicit `Sonderausstellung` / `Dauerausstellung` / `Digitale Ausstellung` controls in the existing Ausstellung editor panel, with the fallback metabox saving the same taxonomy directly
