...
- Imported local candidates for all seven published projects, inserted the optional rail gesture into those candidates, and enabled JSON rendering for visual review.
- Transfer artifacts:
...
- Visually review the seven JSON-backed project pages on localhost, starting with `futura-biennale-2027`, checking the rail and generated context under the meta panel plus no-overlap behavior, then test Set promotion into its `galerie`.
