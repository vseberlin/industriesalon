...
- The merged Atlas/static-map rewrite plan is in `docs/architecture/atlas-static-map-implementation-plan.md`; the related-content editor JS split, static-map DTO boundary, and first interactive Atlas runtime module split are committed locally. The current working tree extracts Atlas place/filter UI into `themes/industriesalon/assets/js/atlas/places.js` on top of the committed store/state split.
