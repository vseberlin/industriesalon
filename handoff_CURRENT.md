...
- Theme-owned Veranstaltung skins are active through the registry/body-class contract: `vortrag`, `lesung`, `gespraech`, and `festival` have concrete CSS/markup treatments from the local skin mockup; `repair` is prepared for future `event.repair_cafe` pages; `workshop` and `praesentation` intentionally remain on the baseline `typografisch` skin until designed.
