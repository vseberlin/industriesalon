...
- Compare `wp iss-graph sync-aliases --dry-run --limit=25` locally and on staging. If reviewed, apply `ops/sql/2026-06-12-graph-alias-backfill-replay.sql` to create the target rollback snapshot, run `wp iss-graph sync-aliases`, then verify post-replay dry-run reports `changed_entities=0`, `removed_names=0`, and `added_names=0`. After that, apply `ops/sql/2026-06-12-graph-canonical-kwo-aeg-organizations.sql` and run graph verify/drift plus `wp iss-graph entity-hygiene-audit --terms=KWO,AEG --limit=25`.
