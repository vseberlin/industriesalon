...
- The generated alias replay data step is prepared in `ops/sql/2026-06-12-graph-alias-backfill-replay.sql`. It creates a targeted rollback snapshot and records the target procedure; the actual mutation remains `wp iss-graph sync-aliases` after code deploy and dry-run comparison.
- The post-replay canonical organization data step for `KWO` and `AEG` is prepared in `ops/sql/2026-06-12-graph-canonical-kwo-aeg-organizations.sql`. It adds hidden curated organization rows and aliases only; it does not merge, delete, or reassign existing content/place/archive graph entities.
- Opt-in graph hygiene migration checks are available: `wp iss-graph drift-check --checks=alias-backfill-replay` verifies generated alias replay is clean, and `wp iss-graph drift-check --checks=canonical-organization-seeds` verifies the curated `KWO` / `AEG` organization rows after their SQL artifact is applied. They are not part of the default drift set yet.
...
- The alias backfill code guard has a prepared SQL/data artifact, but persisted alias replay has not been run locally in this checkpoint. Targets should compare dry-run output first, import the snapshot SQL only after a DB backup, then run the WP-CLI replay and verify a clean post-replay dry-run.
...
- The new migration drift checks intentionally fail on local until the graph data artifacts are applied. Default `wp iss-graph drift-check` remains the stable pre-migration guard.
...
- Compare `wp iss-graph sync-aliases --dry-run --limit=25` locally and on staging. If reviewed, apply `ops/sql/2026-06-12-graph-alias-backfill-replay.sql` to create the target rollback snapshot, run `wp iss-graph sync-aliases`, then verify with `wp iss-graph drift-check --checks=alias-backfill-replay --limit=25`. After that, apply `ops/sql/2026-06-12-graph-canonical-kwo-aeg-organizations.sql` and run `wp iss-graph drift-check --checks=canonical-organization-seeds --limit=25`, full graph verify/drift, and `wp iss-graph entity-hygiene-audit --terms=KWO,AEG --limit=25`.
