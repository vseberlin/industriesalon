...
- The merged Atlas/static-map rewrite plan is in `docs/architecture/atlas-static-map-implementation-plan.md`; the related-content editor JS split, static-map DTO boundary, interactive Atlas runtime module split, Atlas/static-map contract-schema checks, and fullscreen/kiosk Atlas layout states are in local commits. The next Atlas step is a final public-surface audit before deciding whether broader archive/graph API consolidation has a concrete consumer.
