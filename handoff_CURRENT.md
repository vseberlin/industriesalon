...
- GitHub `origin/main` is the exchange point for local/staging agents; use `git status --short --branch`, `git rev-parse --short HEAD`, and `git rev-parse --short origin/main` for the exact current refs before pull/push.
...
- Theme off-canvas menu-shell CSS has been tightened after broad CSS changes: the fixed panel is capped to the visible viewport, WordPress block-gap margins are reset inside the shell, the Institution/Entdecken/Archiv sections share the middle height, and the Heute/next-event status strip sits at the bottom of the viewport. Code/CSS only; no SQL or uploads artifact is required.
...
- SOW stage: Phase 1 is signed off for `ausstellung` authoring and save/reload after editor proof confirmed add/edit/reorder/remove/save/reload behavior. Autosave recovery is intentionally deferred for now; the explicit red `Speichern` button writes the JSON structure directly, and WordPress `Aktualisieren` is only needed for WordPress-owned fields such as title, slug, status, taxonomies, or other metabox data. Phase 2 is signed off for the current Ausstellung pilot boundary: archive-object selection uses the existing bucket-first archive picker, media selection uses `wp.media`, selections persist as typed references, selected objects show titles plus explicit remove controls, and raw IDs are not exposed in the editor tray. The SOW-wide Relation Picker remains deferred until entity-editor work needs it. Phase 3 now has the central skin assignment exposed in the custom composition editor through a single `Darstellung` selector backed by the theme-owned `iss_editorial_format_skins` contract; layout/gesture controls remain out of the editor, section `type` is the editorial gesture, and the theme renders each `gesture x skin` treatment through universal section slots with optional partial fallback. The first skin CSS lives in `themes/industriesalon/assets/css/skins/ausstellung-frauen-im-werk.css` behind conditional skin enqueueing. `typografisch`, `chronik`, and `industrieakte` are registered theme-owned JSON skins; they affect the editorial JSON content body, not the single-template hero. `industrieakte` is the technical/industrial dossier skin for WF/Röhren-like series and reuses the same gesture slots instead of introducing new editor fields. `kicker` is now a first-class JSON section field, imported from old `iss-kicker` markup and rendered through the shared site kicker classes. `quellenauszug` owns the current image/text/quote station treatment and has a scoped text-position flip for left/right source stations; `kapitel`, `leitfrage`, `zitat`, and `fliesstext` have quiet typographic treatments using existing JSON fields and generic renderer markup; `objektfokus` now has a dedicated dark archive-object grid using the existing archive-card renderer; `bildstrecke` and `massstab` adapt the existing `Kinder im Werk` album/stat patterns through the JSON media/body slots; `vollbild` is a finished one-image full-viewport treatment with 16:9 editor guidance and a generic dark ink-panel overlay. Phase 3 still needs curator cleanup, preview proof, and migration/rollback artifact work before signoff.
- Single-Ausstellung template checkpoint: practical `iss/content-meta` now lives in the hero banner slot, not below the hero; local `wp_template` override `26309` is synced to the file template for review. The related tail now uses graph-backed mixed `iss/related-content` with `layoutVariant:"register"`, `source:"entity"`, and `skin:"exhibition"`; `iss-relations` owns the new footer-oriented `register` layout while existing compact `rail` remains available for sidebar/register contexts.
- Local post `21108` (`rohren-fur-die-republik`) now has a compressed `_iss_editorial_ausstellung` document enabled with skin `industrieakte`, 16 sections, and 11 media refs. The paired transfer artifact is `ops/sql/2026-06-23-roehren-republik-editorial-json.sql`; it applies the JSON document and `_iss_editorial_enabled_ausstellung = 1`. The `industrieakte` `massstab` fact sections now render as responsive grid cards instead of a narrow single column.
- Local post `26381` (`kinder-im-werk`) now has a curated `_iss_editorial_ausstellung` document enabled with skin `kinder-im-werk`, 12 sections, 10 media refs, and 3 archive-object refs. The paired transfer artifact is `ops/sql/2026-06-23-kinder-im-werk-editorial-json.sql`; it applies the JSON document and `_iss_editorial_enabled_ausstellung = 1`. The skin CSS lives at `themes/industriesalon/assets/css/skins/ausstellung-kinder-im-werk.css` and currently covers hero suppression, source excerpts, typographic quote, object-focus cards, album copy order, stat gesture, essay, and conclusion treatments.
...
- The `quellenauszug` flip option is code/CSS/editor UI only at this checkpoint. No SQL/upload artifact is required unless a curator saves a flipped section into the `Frauen im Werk` JSON candidate and that changed DB state needs transfer.
...
- Event Drop one-off intake snapshot now wires Uppy Webcam into the existing Dashboard/XHRUpload flow for direct camera capture and native phone/tablet photo/video capture. This is code-only in `ops/event-drop/interface/index.php`; no SQL or uploads artifact is required, and the active target host still needs the snapshot synced to its deployed `shared/event-drop/interface/index.php`.
...
- The `Röhren für die Republik` editorial candidate is DB-backed local state. Use `ops/sql/2026-06-23-roehren-republik-editorial-json.sql` before expecting the JSON-rendered `industrieakte` page on staging; the calculated Leningrad T2 tube totals are derived from article facts and should stay labeled as derived until curator signoff.
- Event Drop camera capture depends on browser camera permissions and a secure context for normal Chrome usage; verify on the HTTPS target after deploying the updated intake snapshot.
- The composition canvas is still v1: basic WordPress media refs are imported/editable/renderable with caption fields and explicit removal controls, archive-object selection is now bucket-first through attached Archivsets with explicit selected-object removal, document skin assignment is exposed through `Darstellung`, and conclusion/decision links are editable and render as a theme-owned button rail. Source links and object/source section semantics still need curator cleanup before the `Frauen im Werk` candidate is ready.
...
- Deploy or sync `ops/event-drop/interface/index.php` to the target Event Drop intake path, then verify the Uppy Dashboard shows the camera source and that captured phone media reaches incoming moderation.
- Complete source/object editing in the composition canvas, then review and clean up the `Frauen im Werk` JSON candidate in the editor UI, verify preview/frontend output for the selected skin and the single-template hero/related tail, and keep legacy fallback available until curator signoff.
- Review the local `Kinder im Werk` JSON conversion in the editor UI and frontend, then use `ops/sql/2026-06-23-kinder-im-werk-editorial-json.sql` before transferring the DB-backed document elsewhere.
- Apply/review `ops/sql/2026-06-23-roehren-republik-editorial-json.sql` on staging, verify `/ausstellungen/rohren-fur-die-republik/`, and curator-review the source-backed/derived facts before treating the `industrieakte` Röhren candidate as production-ready.
...
- Menu-shell viewport checkpoint passed targeted Stylelint for `themes/industriesalon/style.css`, `git diff --check`, and Playwright Chromium/Firefox measurements at `390x640`, `390x844`, and `1280x800`; content fits one viewport, the status strip has zero inherited block margin, and the three labeled middle sections distribute evenly.
- Event Drop Uppy camera checkpoint passed PHP syntax for `ops/event-drop/interface/index.php`, `git diff --check`, and CDN export confirmation that Uppy `v5.2.1` provides `Webcam`. No local deployed staging copy existed at `/srv/industriesalon/stage/shared/event-drop/interface/index.php` during verification.
- Frauen-im-Werk `objektfokus` skin checkpoint passed targeted Stylelint for `themes/industriesalon/assets/css/skins/ausstellung-frauen-im-werk.css`, `git diff --check`, HTTP route smoke for `/ausstellungen/frauen-in-werk/`, and Playwright desktop/mobile measurements showing two archive cards, loaded images, responsive columns, and no horizontal overflow.
- Frauen-im-Werk `bildstrecke`/`massstab` skin checkpoint passed targeted Stylelint, synthetic Playwright desktop/mobile insertion on `/ausstellungen/frauen-in-werk/` using the real theme CSS, and no-overflow checks for the adapted album/stat treatments.
- Ausstellung template/register-related checkpoint passed PHP syntax for touched render/plugin files, targeted JS syntax checks, targeted Stylelint for `single-ausstellung.css`, `patterns.css`, and `ausstellung-frauen-im-werk.css`, targeted PHPCS/PHPStan for touched PHP files, WP-CLI active-template/render checks, HTTP route checks for `/ausstellungen/frauen-in-werk/`, Playwright geometry checks for the centered related register, and `git diff --check`. Local DB template override `26309` was synced to the file template.
- Kinder-im-Werk JSON checkpoint passed PHP syntax for touched theme files, targeted Stylelint for `ausstellung-kinder-im-werk.css`, targeted PHPCS/PHPStan for touched PHP files, WP-CLI read-model checks showing 11 sections and resolved refs, HTTP route checks for `/ausstellungen/kinder-im-werk/`, Playwright desktop/mobile no-overflow checks, and `git diff --check`.
- Röhren-für-die-Republik JSON checkpoint passed SQL reimport via WP-CLI, read-model checks showing 16 sections, 11 resolved media refs, and skin `industrieakte`, HTTP route checks for `/ausstellungen/rohren-fur-die-republik/`, targeted Stylelint for `ausstellung-industrieakte.css`, Playwright desktop/mobile measurements showing fact cards render in multi-column desktop grids and single-column mobile grids without horizontal overflow, and `git diff --check`.
- Typografisch/Chronik skin checkpoint passed PHP syntax for the theme skin registry/enqueue edits, targeted Stylelint for both new skin files, registry smoke showing `typografisch` and `chronik`, and Playwright desktop/mobile checks by swapping each skin stylesheet/class into `/ausstellungen/frauen-in-werk/` without DB writes. The proof confirms CSS loading, object grids, synthetic stat sections, hidden archive excerpts, and no horizontal overflow; it is not a real curator-content signoff.
- Local dirt outside the Röhren checkpoint may include the Event Drop snapshot files `HANDOFF_EVENT_DROP_ONEOFF.md` and `ops/event-drop/interface/index.php`, plus untracked `theme2.json` and `iss-exhibition-composition-add.md`; do not stage them unless that work is intentionally checkpointed.
