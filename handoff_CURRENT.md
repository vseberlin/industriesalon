...
- The alias replay and KWO/AEG canonical organization artifacts are now applied on staging, but not production. Production must still apply them in order after a DB backup.
...
- For production transfer, take a target DB backup, apply the current programme/template SQL artifacts, apply `ops/sql/2026-06-12-graph-alias-backfill-replay.sql`, run `wp iss-graph sync-aliases`, verify `wp iss-graph drift-check --checks=alias-backfill-replay --limit=25`, then apply `ops/sql/2026-06-12-graph-canonical-kwo-aeg-organizations.sql` and verify `wp iss-graph drift-check --checks=canonical-organization-seeds --limit=25`.
- For remaining graph hygiene, do not auto-merge. Next review targets are `Industriesalon Schöneweide` organization variants and `WF`, both requiring curatorial decision.
