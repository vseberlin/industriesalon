...
- Domain plugin ownership is now `iss-content` for CPT/editor contracts and Führung module data, `iss-occurrences` for dated occurrence projection plus SuperSaaS ingestion/tour-slot reads, `iss-frontend` for programme/timeline/browser rendering, `iss-commerce-lite` for booking/order request intake, `iss-archive` for archive runtime, and `iss-graph` for graph/search/contracts. The theme owns public templates/skins.
- Occurrence rows are source-post keyed only. Schema v8 removes retired graph columns/indexes and adds `is_open_ended`; open-ended rows use `ends_at = NULL`, not a `2099-12-31` sentinel. Graph-facing occurrence reads translate entity filters to source post filters at the `iss-graph` facade boundary and compute entity IDs on read.
- Occurrence source projection is provider-class based inside `iss-occurrences`: `veranstaltung`, `ausstellung`, and opt-in `projekt` are editorial WordPress source providers; `fuehrung` rows are external SuperSaaS slot projections linked back to parent Führung posts.
...
- `iss_occurrences_public_query_ready()` now checks occurrence table readiness, not row count, so valid empty projection tables do not disable frontend queries.
- `iss-occurrences` no longer calls frontend horizon/date formatting helpers from its service; occurrence query horizon and date-label formatting are owned by the occurrence service.
- SuperSaaS settings, public tour-slot reads, slot cache helpers, occurrence sync cron, series mapping, and sync admin now live in `iss-occurrences`; the operational Tools page is `tools.php?page=iss-occurrences-sync`. `iss-commerce-lite` owns booking/order writes only, and `iss-frontend/modules/programme` should stay render/query-only.
- SuperSaaS settings now use `iss_supersaas_*` option/function naming; the retired `is_saas_settings` option migrates to `iss_supersaas_settings` and is guarded by occurrence drift.
...
- `iss-commerce-lite` still keeps internal `iss_payments_lite_*` function/option/table naming for compatibility with the current request table, hooks, REST namespace, and verification aliases.
...
- `/ausstellungen/` uses the dedicated `industriesalon/ausstellungen-browser` and WP_Query availability filters backed by `iss_public_overview_enabled`. Programme/calendar projection is separate through `iss_programme_enabled`, so Dauer/Digital Ausstellungen can opt into occurrence rows explicitly instead of being hard-excluded by type.
...
- The local occurrence drift guard also rejects retired `2099-12-31` sentinel dates, retired `iss_timeline_enabled` meta, and legacy `iss_calendar_item` posts/options/meta as invalid drift, not compatibility inputs.
...
- Current local occurrence ownership cleanup verification passed: PHP syntax, JS syntax, JS lint, PHPCS target, PHPStan target, occurrence schema/provider/admin probes, programme visibility migration probe (`iss_timeline_enabled=0`, Dauer/Digital overview preserved, programme opt-in zero), SuperSaaS settings migration probe (`iss_supersaas_settings` present, `is_saas_settings` missing), `wp iss-occurrences verify`, `wp iss-occurrences sync --source=wp`, `wp iss-occurrences drift-check --limit=25`, sentinel-row probe, grouped occurrence query smoke, readiness/table-empty probe, SuperSaaS route/settings/cron probes, `wp iss-content tours-drift-check --limit=25`, `wp iss-frontend ausstellungen-audit --strict`, graph occurrence/entity-occurrence/tour-slot/availability facade compares, default graph drift, `wp iss-commerce-lite verify`, active plugin list, and `git diff --check`.
