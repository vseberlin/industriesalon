...
- Local programme cleanup artifacts exist and must travel with the matching code if deployed: `ops/sql/2026-06-11-ausstellung-availability-cleanup.sql`, `ops/sql/2026-06-11-strict-programme-toggle-backfill.sql`, and `ops/sql/2026-06-12-legacy-occurrence-origin-purge.sql`.
...
- For production transfer, take a target DB backup, apply the three SQL artifacts, run occurrence migrate/sync if needed, then run graph/occurrence verify and drift checks on the target.
