...
- Local `main` is current for the editorial-platform v1 SOW slice once this checkpoint is pushed to `origin/main`; use `git log -1` for the exact commit.
...
- Editorial-platform v1 has its first code slice: `iss-editorial` is engine-only, `iss-content` opts `ausstellung` into the `OrderedFormat` pilot, archive-object references use typed refs through a normalized bucket-first archive picker modal, `ausstellung` now uses a custom main-canvas composition editor instead of the Gutenberg block editor/sidebar, and theme-owned JSON rendering is behind `_iss_editorial_enabled_ausstellung` so existing Gutenberg output remains the default fallback.
- SOW stage: Phase 1 is functionally in place for `ausstellung` authoring and save/reload; the archive/media picker portion of Phase 2 is usable for this pilot; Phase 3 is not complete because the skin model, skin override resolution in the theme, and editor-facing skin control decisions are still undefined.
...
- Local post `26287` (`frauen-in-werk`) has `_iss_editorial_ausstellung` candidate document with 7 sections and 6 media refs. `_iss_editorial_enabled_ausstellung` is local review state and is currently enabled in the local DB; turn it off before treating Gutenberg as the visible local fallback again. Editorial JSON meta writes now slash encoded JSON before `update_post_meta()` so paragraph breaks survive storage.
...
- The `Frauen im Werk` editorial candidate is DB-backed local state. Create a paired SQL artifact before transferring that candidate document to another environment.
- The composition canvas is still v1: basic WordPress media refs are imported/editable/renderable with caption fields and explicit removal controls, and archive-object selection is now bucket-first through attached Archivsets. Source links, object/source section semantics, and research navigation links still need curator cleanup before the `Frauen im Werk` candidate is ready.
...
- Define Ausstellung skins and skin-control decisions next: name the first skin(s), decide whether editors choose only semantic skins or also variants, decide where controls live in the main canvas, and then wire theme partial override resolution before curator review of the `Frauen im Werk` candidate.
- Complete source/object/research-link editing in the composition canvas, then review and clean up the `Frauen im Werk` JSON candidate in the editor UI, verify preview/frontend output, and keep legacy fallback available until curator signoff.
...
- Exit checkpoint: `origin/main` is current at `fa75ea0`; remaining local dirt is limited to untracked `theme2.json` and `iss-exhibition-composition-add.md`.
