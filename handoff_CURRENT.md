...
- Local programme cleanup artifacts exist and must travel with the matching code if deployed: `ops/sql/2026-06-11-ausstellung-availability-cleanup.sql`, `ops/sql/2026-06-11-strict-programme-toggle-backfill.sql`, `ops/sql/2026-06-12-legacy-occurrence-origin-purge.sql`, `ops/sql/2026-06-12-supersaas-past-occurrence-reactivation.sql`, and `ops/sql/2026-06-12-tour-template-collapse.sql`.
...
- Führung single-template collapse checks passed locally: `single-tour` is theme-backed, `single-tour-on-demand` is no longer a block-template source, published Führung template assignments were rewritten to `single-tour`, hybrid/on-demand tour URLs render through the canonical template without empty public-date panels, and `wp iss-fuehrungen drift-check` catches stale retired-template assignments.
