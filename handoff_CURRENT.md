...
- Remaining apt updates after the targeted pass are Docker packages only (`docker-ce`, `docker-ce-cli`, `docker-ce-rootless-extras`); they were not applied because Docker updates can affect running containers and were outside the SSH/plugin request.
