...
- Applied Ausstellung content now live on staging: `/ausstellungen/kinder-im-wf/`, `/ausstellungen/kinder-im-werk/`, and `/ausstellungen/frauen-in-werk/`. The refreshed upload artifact was extracted into the shared uploads bind mount and all 91 manifest files were verified.
...
- The Ausstellung SQL artifacts are narrow content/custom-table transfer files. On staging, the `Kinder im WF`, `Kinder im Werk`, and corrected `Frauen im Werk` artifacts have been applied with the refreshed upload archive.
...
- 2026-06-10 corrected staging Frauen transfer verification: pulled commit `9c89121`; upload checksum passed; rollback DB and upload archives were created with SHA-256 checksums; normalized applied SQL contained no `192.168.2.31` URLs; all 91 upload manifest files existed after extraction; post `26287`, WebP attachment rows, Archivset `27`, six members, and the archive-material link exist; imported rows checked returned `0` local dev URLs; affected DB tables passed `CHECK TABLE`; `/`, `/ausstellungen/kinder-im-wf/`, `/ausstellungen/kinder-im-werk/`, and `/ausstellungen/frauen-in-werk/` returned `200`; sampled Frauen WebP files returned `200 image/webp`; containers remained healthy.
