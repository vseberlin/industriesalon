...
- The merged Atlas/static-map rewrite plan is in `docs/architecture/atlas-static-map-implementation-plan.md`; the related-content editor JS split, static-map DTO boundary, and interactive Atlas runtime module split are committed locally through detail/story/relation rendering. The current working tree extracts marker icon/map-marker orchestration into `themes/industriesalon/assets/js/atlas/markers.js`, leaving `schoneweide.js` as bootstrap coordinator.
...
- Continue Atlas cleanup from `docs/architecture/atlas-static-map-implementation-plan.md`: add focused contract/schema tests for Atlas REST payloads and static map inputs.
