...
- The SuperSaaS sync tool remains an operational Tools page
  (`tools.php?page=iss-programm-sync`) guarded by the shared sync capability.
- Ausstellung availability is edited on the Ausstellung parent object through
  existing type, date, visibility, and availability-signal controls. It is not a
...
- Future JSON-LD work must consume the facade schema intent and must not emit
  Event schema for availability-only Ausstellung records without concrete dated
  occurrences.
