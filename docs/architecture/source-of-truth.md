...
| Place narratives and epochs | Enabled `iss-editorial` Place JSON per post; legacy epoch rows only for disabled Places | `wp iss-register place-editorial-check`, Place editor, frontend dossier | Epoch/state tables are derived projections for enabled Places |
