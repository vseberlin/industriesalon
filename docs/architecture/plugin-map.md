...
| --- | --- | --- | --- | --- | --- |
| `iss-content-model` | shared CPT/editor/data contracts, Ausstellung type/source meta, Video CPT block data | WordPress CPT/block APIs | dynamic blocks and editor support | public page skins owned by theme | `includes/`, `blocks/`, `assets/` |
...
| `iss-graph` | entities, names, identifiers, evidence refs, relations, profiles, transcript evidence bridge, SQL search projection | custom graph/search tables, WP posts | resolver, resolver-backed label creation, search provider, graph/profile services | theme search UI composition | `includes/core.php`, `includes/search-*` |
