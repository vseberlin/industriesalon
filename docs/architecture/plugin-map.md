...
| `iss-graph` | entity-kind registry, offer subtype contract bridge, entities, names, identifiers, evidence refs, relations, editorial relation signals, profiles, transcript evidence bridge, SQL search projection, read-only `/iss/v1` facade | custom graph/search tables, WP posts, `iss-occurrences` query API when active | resolver, resolver-backed label creation, search provider, graph/profile services, editorial signal controls, public contract facade | theme search UI composition, occurrence storage, offer/editorial CPT storage | `includes/entity-kinds.php`, `includes/core.php`, `includes/facade-rest.php`, `includes/search-*`, `includes/editorial-signals-*` |
